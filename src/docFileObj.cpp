...
docFileObj::docFileObj(const char* filePath) {
	...
	Serial.print("docFileObj::docFileObj() given filePath [");
	...
	if (!docFilePath) {
		Serial.println("docFileObj() - heapStr() failed!");
	}
}
...
docFileObj::~docFileObj(void) {
	...
	if (autoGenFile) {				// If this was originally an auto generated file..
		Serial.println("removing autoGen file.");
...
bool docFileObj::openDocFile(fOpenMode openMode) {
	...
	Serial.print("docFileObj::openDocFile() - [");
	Serial.print(docFilePath);
	Serial.println("]");
	...
	if (docFilePath) {															// If we have a non-NULL path..
		if (openMode==fWrite) {											// If they want to edit this file..
			Serial.println("Trying to open a file to write.");
			...
			if (mode==fOpenToRead) {											// If we're already open to read..
				...
				if (ourFile) {														// If we were able to open the file..
					...
					Serial.println("openDocFile() Opening for fRead, FAILED");
				}
			}
		}
	} else {
		Serial.println("MISSING docFilePath!");
	}
	return success;
}
...
char* docFileObj::getFolder(void) {
	...
	return returnBuffer;								// Return a pointer to the resultant path string.
}
...
bool docFileObj::seek(uint32_t index) {
	...
	if (mode != fClosed) {
		...
		Serial.println("docFileObj::seek() called on closed file.");
		return false;
	}
}
